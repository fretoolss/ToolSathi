  ...
  return (
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div>
        ...
        <p className="text-slate-500 dark:text-slate-400 mt-1">Everyday calculators and converters.</p>
      </div>
        ...
        {category.tools.map(tool => {
          ...
          return (
            <Link 
              key={tool.name} 
              to={tool.path}
              className="bg-white dark:bg-slate-950 p-6 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-md hover:border-amber-500/50 transition-all group"
            >
              <div className="bg-amber-50 dark:bg-amber-500/10 w-12 h-12 rounded-xl flex items-center justify-center mb-4 group-hover:scale-110 transition-transform">
